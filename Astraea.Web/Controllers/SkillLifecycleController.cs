...
public sealed class SkillLifecycleController(AstraeaDbContext db, IUnitOfWork uow) : ControllerBase
{
    ...
    [HttpGet("archived")]
    public async Task<IReadOnlyCollection<ArchivedSkillVm>> Archived(CancellationToken ct)
    {
        ...
        return skills
            .Select(x => new ArchivedSkillVm(
                x.Id,
                x.Title,
                x.ConstellationCategory,
                x.EaseFactor,
                x.CurrentIntervalDays,
                x.CanvasX,
                x.CanvasY,
                Math.Round(
                    Math.Clamp(
                        100 * Math.Exp(-Math.Max(0, (now - x.LastReviewedUtc).TotalDays) / Math.Max(x.CurrentIntervalDays, 1)),
                        0,
                        100),
                    1)))
            .ToArray();
    ...
    public async Task ClearArchived(CancellationToken ct)
    {
        ...
        db.SkillPrerequisites.RemoveRange(
            db.SkillPrerequisites.Where(x => ids.Contains(x.SkillId) || ids.Contains(x.PrerequisiteSkillId)));
    ...
    {
        var skill = await db.Skills.SingleOrDefaultAsync(x => x.Id == id && x.LearnerId == LearnerId, ct)
